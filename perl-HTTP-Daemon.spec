%{?scl:%scl_package perl-HTTP-Daemon}

Name:           %{?scl_prefix}perl-HTTP-Daemon
Version:        6.06
Release:        3%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(HTTP::Date) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Request) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Response) >= 6
BuildRequires:  %{?scl_prefix}perl(HTTP::Status) >= 6
BuildRequires:  %{?scl_prefix}perl(IO::Socket::IP)
BuildRequires:  %{?scl_prefix}perl(LWP::MediaTypes) >= 6
...
Requires:       %{?scl_prefix}perl(HTTP::Date) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Request) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Response) >= 6
Requires:       %{?scl_prefix}perl(HTTP::Status) >= 6
Requires:       %{?scl_prefix}perl(LWP::MediaTypes) >= 6
Requires:       %{?scl_prefix}perl(Sys::Hostname)
Conflicts:      %{?scl_prefix}perl-libwww-perl < 6

# Remove underspecified dependencies
%global __requires_exclude %{?__requires_exclude:%{__requires_exclude}|}^%{?scl_prefix}perl\\(HTTP::(Date|Request|Response|Status)|LWP::MediaTypes\\)$
...
%changelog
* Thu Feb 13 2020 Petr Pisar <ppisar@redhat.com> - 6.06-3
- Import to SCL
